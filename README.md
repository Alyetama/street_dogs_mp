...
| `tools/detect/` | Everything downstream of the harvest: the YOLO detection sweep (`sweep.py`, `engine.py`, `store.py`), the two classifier gates and their stores (`gate_store.py`, `leash_store.py`), dataset builders (`build_dogdet_v3.py`, `build_review_set.py`, `rebuild_crop_dataset.py`), the one-command dataset build and training launcher behind the dashboard's `/train` page (`build_dataset.py`, `train_model.py`), the false-negative audit (`fn_audit.py`), the LLM annotator (`llm_annotate.py`), and `tests/` — including `adv_no_hardcoded_paths.py`, the leak check run before every push. |
| `tools/dashboard/` | The self-hosted dashboard (`dashboard.py`) and the human annotation pages it serves: `/audit/review`, `/audit` (`audit.py`), `/datasets` (`datasets.py`), `/llm` (`llm_page.py`), `/train` (`train_page.py`), plus `jobs.py` (background work that outlives the browser tab), `map_layers.py` (the map's model layers), `training_tracker.py` and `country_index.py`. |
...
python backfill_missing.py \
    ...
    --out-dir /mnt/work/grid_runs --image-dir /mnt/jpgs \
