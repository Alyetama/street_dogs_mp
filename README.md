...
| `tools/dashboard/` | The self-hosted dashboard (`dashboard.py`) and the human annotation pages it serves: `/audit/review`, `/audit` (`audit.py`), `/datasets` (`datasets.py`), `/llm` (`llm_page.py`), plus `map_layers.py` (the map's model layers), `training_tracker.py` and `country_index.py`. |
