...
def get_image_topology(west, south, east, north, region_dir, sub_id, session,
    ...
    # 1. LAZY EVALUATION: Generator instead of list()
...
def process_region(west, south, east, north, unique_region_id, run_name):
    ...
    for i, (sw_lon, sw_lat, ne_lon, ne_lat) in enumerate(sub_bboxes):
        ...
        # Step D: Process Metadata to CSV
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Mapillary Ground Animal API Downloader (SLURM Version)")
    parser.add_argument(
        'grid_csv_file',
        type=str,
    ...
    # Fetch only the single row corresponding to this SLURM task
